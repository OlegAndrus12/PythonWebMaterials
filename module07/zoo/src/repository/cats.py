...
from src.schemas.cats import CatModel, CatStatusVaccinated
...
async def update_cat(body: CatModel, cat_id: int, db: Session):
...
async def update_vaccinated_cat(body: CatStatusVaccinated, cat_id: int, db: Session):
    cat = db.query(Cat).filter_by(id=cat_id).first()
    if cat:
        cat.vaccinated = body.vaccinated
        db.commit()
